...
//        return Input::all();
        ...
        $bng_data = ["title" => "বাংলাদেশ আনসার ও গ�?রাম প�?রতিরক�?ষা বাহিনী",
            "id_no" => "আইডি নং",
            "name" => "নাম",
            "rank" => "পদবী",
            "bg" => "রক�?তের গ�?র�?প",
            ...
            "ed" => "শেষের তারিখ",
            "bs" => "বাহকের স�?বাক�?ষর",
            "is" => "কর�?তৃকারীর স�?বাক�?ষর",
            "footer_title" => "প�?রদানকারী কর�?তৃপক�?ষ : বাংলাদেশ আনসার ও ভিডিপি"];
        $eng_data = [
...
//            return View::make('HRM::Report.ansar_id_card_font',['rd' => $report_data, 'ad' => $ansar, 'id' => Carbon::parse( $issue_date)->format("d/m/Y"), 'ed' => Carbon::parse( $expire_date)->format("d/m/Y"), 'type' => $type]);
    ...
    public function getAnserTransferHistory(Request $request)
    {
        ...
        if ($validation->fails()) {
            ...
            echo '<pre>';
            //print_r($result["cEmbodiment"]); exit;
            ...
            if ($result["lEmbodiment"]->count() == 0) {
                foreach($result["lEmbodiment"] as $index=>$lEmbodiment){
                    $date_time_array[$index]['time'] = $lEmbodiment->joining_date;
                    $date_time_array[$index]['type'] = 'embodiment';
                    $tracker++;
                }
            }
            ...
            if ($result["lFreeze"]->count() == 0) {            
                ...
                foreach($result["lFreeze"] as $index2=>$lFreeze){
                    $date_time_array[$index + $index2]['time'] = $lFreeze->freez_date;
                    $date_time_array[$index + $index2]['type'] = 'freez';
                    $tracker++;
                }
            }
...
//            }
            ...
            if ($result["transfer"]->count() == 0) {
                ...
                foreach($result["transfer"] as $index4=>$transfer){
                    $date_time_array[$index + $index4]['time'] = $transfer->present_kpi_join_date;
                    $date_time_array[$index + $index4]['type'] = 'transfer';
                     $tracker++;
                }
            }
...
//            return DB::getQueryLog();
            //return $b;
             ...
             return Response::json(['ansar' => $ansar, 'transfer_data' => $result["transfer"], 'pi' => file_exists($ansar->profile_pic)]);
...
//        return Input::all();
        ...
        $offer_received_query = DB::table('tbl_sms_receive_info')
            ...
            ->unionAll(DB::table('tbl_sms_send_log')
                ...
                ->select('tbl_sms_send_log.offered_date', 'tbl_ansar_parsonal_info.mobile_no_self', 'u.unit_name_bng as home_district', 'tbl_ansar_parsonal_info.ansar_name_eng', 'tbl_units.unit_name_bng', 'tbl_ansar_parsonal_info.ansar_id', 'tbl_designations.code', 'tbl_sms_send_log.action_date as sms_received_datetime', 'tbl_sms_send_log.memo_id'));
...
//        return DB::getQueryLog();
    ...
    public function viewAnsarHistoryReport(Request $request)
    {
        ...
        $result["cInfoHistoryLog"] = $ansar->personalInfoLog()->orderBy("log_id", "desc")->get();
