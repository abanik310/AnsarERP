...
//                    'tbl_units.unit_name_bng', 'tbl_units.id as unit_id', 'tbl_ansar_parsonal_info.data_of_birth', 'tbl_designations.name_bng', 'tbl_ansar_parsonal_info.mobile_no_self')->first();
            ...
            $ansarPersonalDetail = DB::table('tbl_ansar_parsonal_info')
                ...
                ->select('tbl_ansar_parsonal_info.ansar_name_bng', 'tbl_ansar_parsonal_info.profile_pic', 'tbl_ansar_parsonal_info.ansar_id',
                    ...
                    'tbl_ansar_bank_account_info.bank_name', 'tbl_ansar_bank_account_info.prefer_choice', 'tbl_ansar_bank_account_info.mobile_bank_type', 'tbl_ansar_bank_account_info.account_no', 'avub_share_id', 'tbl_thana.thana_name_bng', 'tbl_blood_group.blood_group_name_bng',
                    DB::raw('ROUND((CHAR_LENGTH(REPLACE(offer_type,",",""))-CHAR_LENGTH(REPLACE(REPLACE(offer_type,",",""),"DG","")))/CHAR_LENGTH("DG"))+ROUND((CHAR_LENGTH(REPLACE(offer_type,",",""))-CHAR_LENGTH(REPLACE(REPLACE(offer_type,",",""),"CG","")))/CHAR_LENGTH("CG"))+ROUND((CHAR_LENGTH(REPLACE(offer_type,",",""))-CHAR_LENGTH(REPLACE(REPLACE(offer_type,",",""),"GB","")))/CHAR_LENGTH("GB")) as go_offer_count'),
                    DB::raw('ROUND((CHAR_LENGTH(REPLACE(offer_type,",",""))-CHAR_LENGTH(REPLACE(REPLACE(offer_type,",",""),"RE","")))/CHAR_LENGTH("RE")) as re_offer_count'))->first();
...
//            return $ansar->getStatus();
            ...
            } if ($panel_info->come_from == 'Offer Block') {
				
                $blocked_ansar =  OfferBlockedAnsar::where('ansar_id', $ansar_id)->first();
                $blocked_ansar->status = "blocked";
                $blocked_ansar->unblocked_date = null;
                $blocked_ansar->save();
                
                $panel_info->saveLog("Offer Block", Carbon::now(), $cancel_panel_comment);
                $ansar->update([
                    'offer_block_status' => 1,
                    'pannel_status' => 0
                ]);
                
                CustomQuery::addActionlog(['ansar_id' => $request->input('ansar_id'), 'action_type' => 'DIRECT CANCEL PANEL', 'from_state' => 'PANEL', 'to_state' => 'Offer Block', 'action_by' => auth()->user()->id]);
            }else {
                   ...
                   // $blocked_ansar = OfferBlockedAnsar::findOrFail($ansar_id);  
                    ...
                    if ($modified_direct_panel_date->lte(Carbon::now())) {
                        ...
                        AnsarFutureState::create([
                            ...
                            'activation_date' => $modified_direct_panel_date,
                            'action_by' => Auth::user()->id,
                            'from_status' => 'Offer_blocked',
                            'to_status' => 'Panel',
                        ]);
                    }
                    CustomQuery::addActionlog(['ansar_id' => $request->input('ansar_id'), 'action_type' => 'DIRECT PANEl', 'from_state' => 'Offer Block', 'to_state' => 'PANELED', 'action_by' => auth()->user()->id]);
                    break;
                default:
 ...
 */
    ...
    public function directOfferSend(Request $request)
    {
        ...
        if ($valid->fails()) {
            //return $valid->messages()->toJson();
			//return Response::json(['status' => false, 'type'=> 'validation_error', 'message' => $valid->messages()->toJson()]);
			return Response::json(['status' => false, 'message' => 'All the field is required.']);
			   //return response(collect(['status' => false, 'type' => 'error', 'message' => $valid->messages()->toJson()])->toJson(), 200, ['Content-Type' => 'application/json']);
        }
        DB::beginTransaction();
        try {
            $a = PersonalInfo::where('ansar_id', $request->ansar_id)->first();
...
//                return DB::getQueryLog();
    ...
    public function viewUserRequestLog(Request $request)
    {
        if (strcasecmp($request->method(), "post") == 0) {
            $requestDump = RequestDumper::with("user");
            if (isset($request->user) && !empty($request->user)) {
                if (is_numeric($request->user)) {
                    $requestDump->where("user_id", $request->user);
                } else {
                    $requestDump->whereHas("user", function ($q) use ($request) {
                        $q->where("user_name", $request->user);
                    });
                }
            }
            if (isset($request->ip) && !empty($request->ip)) {
                $requestDump->where("request_ip", $request->ip);
            }
            if (isset($request->url) && !empty($request->url)) {
                $requestDump->where("request_url", "like", "%" . $request->url . "%");
            }
            if (isset($request->sDate) && !empty($request->sDate) && isset($request->eDate) && !empty($request->eDate)) {
                $requestDump->whereBetween("created_at", [$request->sDate . " 00:00:00", $request->eDate . " 22:59:59"]);
            }
            return Response::json(["success" => $requestDump->orderBy("id", "desc")->get()]);
        }
        return View::make('HRM::Dgview.user_request_log');
    }
}
