...
//        return DB::getQueryLog();
    ...
    public function loadAnsarForEmbodiment(Request $request)
    {
        ...
        foreach ($detail as $d) {
            ...
            $data = PanelInfoLogModel::where('ansar_id', $d->ansar_id)->first();
            if (!$data) {
            ...
            if ($data) {
                ...
                $pi['memorandum_id'] = $data->old_memorandum_id;
                array_push($apd, $pi);
            }
        }
...
//        return $request->all();
        ...
        if ($valid->fails()) {
           ...
           //return Response::json(['status' => false, 'message' => $valid->messages()->toArray()]);
...
//        }
    ...
    {  
        $extended_period_year = $request->input('extended_period_year');
        $extended_period_month = $request->input('extended_period_month');
        $extended_period_day = $request->input('extended_period_day');
        $rules = [
            ...
            'service_extension_comment' => 'required',           
        ...
        if($extended_period_month != ''){
            $rules['extended_period_month'] = 'required|numeric|min:1|max:12';
            $message['extended_period_month.min'] = 'Month Cannot be less than 1';
            $message['extended_period_month.max'] = 'Month Cannot be more than 12';
        }
        
        
         if($extended_period_day != ''){
            $rules['extended_period_day'] = 'required|numeric|min:1|max:30';
            $message['extended_period_day.min'] = 'Extended Period Cannot be less than 1 Day';
            $message['extended_period_day.max'] = 'Extended Period Cannot be more than 30 Days';
        }
        if($extended_period_month != ''){
            $rules['extended_period_month'] = 'required|numeric|min:1|max:12';
            $message['extended_period_month.min'] = 'Month Cannot be less than 1';
        ...
        if ($ansarExist == 1) {
            ...
            try {
                ...
                $serviceExtenstionEntry->new_extended_date = $embodiment_info->service_ended_date;
                
                if($extended_period_year != ''){
                    $serviceExtenstionEntry->new_extended_date = Carbon::parse($serviceExtenstionEntry->new_extended_date)->addYear($extended_period_year);
                }
                
                
                if($extended_period_month != ''){
                    $serviceExtenstionEntry->new_extended_date = Carbon::parse($serviceExtenstionEntry->new_extended_date)->addMonth($extended_period_month);
                }
                
                if($extended_period_day != ''){
                    
                    $serviceExtenstionEntry->new_extended_date = Carbon::parse($serviceExtenstionEntry->new_extended_date)->addDay($extended_period_day);
                }
                
                if($extended_period_day == '' && $extended_period_month == '' && $extended_period_year == ''){
                    
                             return Redirect::route('service_extension_view')->with('error_message', 'Please Provide a Valid Extension Period!');
                }
