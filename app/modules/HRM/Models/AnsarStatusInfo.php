...
class AnsarStatusInfo extends Model
{
   ...
   // protected $appends = ['unit_data'];
     ...
     function freeze()
    {
     ...
     function offer_block()
    {
        return $this->hasOne(OfferBlockedAnsar::class, 'ansar_id', 'ansar_id');
    }
    
    public function get_unit($ansar_id)
    {
       $data =  OfferBlockedAnsar::with('unit')->where('ansar_id', $ansar_id)->first();
       //return $data; 
       
       //public function getAnsweredAttribute() {
     return $this->attributes['unit_data'] = $data;
       //}
    }
    
    public function get_last_offer($ansar_id){
       $data =  OfferSmsLog::with('unit')->where('ansar_id', $ansar_id)->latest('id')->first();
       return $this->attributes['unit_data'] = $data;
   }
